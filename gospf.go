...
const (
	GOSPF_IMPORT_PATH = "github.com/hubply/gospf"
)

type gospfLogs struct {
	c gocolorize.Colorize
	w io.Writer
}

func (r *gospfLogs) Write(p []byte) (n int, err error) {
...
var (
	...
	// gospf installation details
	gospfPath string // e.g. "/Users/robfig/gocode/src/gospf"
	...
	error_log = gospfLogs{c: colors["error"], w: os.Stderr}
...
// Init initializes gospf -- it provides paths for getting around the app.
//
// Params:
//   mode - the run mode, which determines which app.conf settings are used.
//   importPath - the Go import path of the application.
//   srcPath - the path to the source directory, containing gospf and the app.
...
func Init(mode, importPath, srcPath string) {
	...
	var gospfSourcePath string // may be different from the app source path
	if SourcePath == "" {
		gospfSourcePath, SourcePath = findSrcPaths(importPath)
	} else {
		// If the SourcePath was specified, assume both gospf and the app are within it.
		SourcePath = path.Clean(SourcePath)
		gospfSourcePath = SourcePath
		packaged = true
	}

	gospfPath = path.Join(gospfSourcePath, filepath.FromSlash(GOSPF_IMPORT_PATH))
	...
	ConfPaths = []string{
		...
		path.Join(gospfPath, "conf"),
	}

	TemplatePaths = []string{
		ViewsPath,
		path.Join(gospfPath, "templates"),
	}
	...
	CookiePrefix = Config.StringDefault("cookie.prefix", "gospf")
	...
	Initialized = true
	INFO.Printf("Initialized gospf v%s (%s) for %s", VERSION, BUILD_DATE, MINIMUM_GO)
}
...
func getLogger(name string) *log.Logger {
	...
	var newlog gospfLogs

	switch output {
	case "stdout":
		newlog = gospfLogs{c: colors[name], w: os.Stdout}
		logger = newLogger(&newlog)
	case "stderr":
		newlog = gospfLogs{c: colors[name], w: os.Stderr}
		logger = newLogger(&newlog)
	default:
		if output == "off" {
			output = os.DevNull
...
// findSrcPaths uses the "go/build" package to find the source root for gospf
// and the app.
func findSrcPaths(importPath string) (gospfSourcePath, appSourcePath string) {
	var (
	...
	gospfPkg, err := build.Import(GOSPF_IMPORT_PATH, "", build.FindOnly)
	if err != nil {
		ERROR.Fatalln("Failed to find gospf with error:", err)
	}
	return gospfPkg.SrcRoot, appPkg.SrcRoot
}
...
func addModule(name, importPath, modulePath string) {
	...
	if importPath == Config.StringDefault("module.testrunner", "github.com/gospf/modules/testrunner") {
...
func CheckInit() {
	if !Initialized {
		panic("gospf has not been initialized!")
	}
}
