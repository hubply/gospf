...
// For example:
//
//     func (c Users) ShowUser(id int) gospf.Result {
//     	 user := loadUser(id)
...
func (c *Controller) SetAction(controllerName, methodName string) error {
	...
	if c.Type, ok = controllers[strings.ToLower(controllerName)]; !ok {
		return errors.New("gospf/controller: failed to find controller " + controllerName)
	}
	if c.MethodType = c.Type.Method(methodName); c.MethodType == nil {
		return errors.New("gospf/controller: failed to find action " + methodName)
	}
...
// Specifically, it sets all *gospf.Controller embedded types to the provided controller.
...
// Register a Controller and its Methods with gospf.
