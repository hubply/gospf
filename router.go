...
func (router *Router) Reverse(action string, argValues map[string]string) *ActionDefinition {
	actionSplit := strings.Split(action, ".")
	if len(actionSplit) != 2 {
		ERROR.Print("gospf/router: reverse router got invalid action ", action)
		return nil
	}
	...
	for _, route := range router.Routes {
		...
		for i, el := range pathElements {
			...
			if !ok {
				val = "<nil>"
				ERROR.Print("gospf/router: reverse route missing route arg ", el[1:])
			}
			pathElements[i] = val
