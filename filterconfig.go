...
// Insert:
//   FilterAction(App.Action).
//     Insert(OtherFilter, gospf.BEFORE, SessionFilter)
//
...
//   gospf.FilterAction(App.Index).
//     Insert(MyFilter, gospf.BEFORE, gospf.ActionInvoker).
//     Insert(MyFilter2, gospf.AFTER, gospf.PanicFilter)
...
func (conf FilterConfigurator) getChain() []Filter {
	...
	if filters = getOverrideChain(conf.controllerName, conf.key); filters == nil {
		...
		if filters == nil {
			panic("FilterConfiguringFilter not found in gospf.Filters.")
		}
	}
	return filters
}
